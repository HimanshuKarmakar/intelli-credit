...
async function renderStep6(container) {
  ...
  container.innerHTML = `
    ...
    <div class="wizard-actions">
      <button class="btn btn-ghost" onclick="prevStep()">← Back</button>
      <button class="btn btn-success" onclick="navigateTo('dashboard')">✓ Finalize & Return to Dashboard</button>
    </div>`;
}
...
async function generateCAM() {
  if (isBackendAvailable) {
    ...
    alert('CAM document downloaded successfully!');
    return;
  }
...
async function renderAuditTrail() {
  ...
  if (entries.length === 0) {
    timeline.innerHTML = '<div class="empty-state"><div class="empty-icon">📋</div><h3>No audit entries yet</h3><p>Start a credit assessment to generate an audit trail.</p></div>';
    return;
  }
  timeline.innerHTML = entries.map((entry, i) => {
    ...
    return `<div class="audit-entry ${i < entries.length - 1 ? 'complete' : ''}">
