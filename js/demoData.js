// ============================================
// INTELLI-CREDIT — Demo Data
// 5 Indian Corporate Cases
// ============================================
...
const DEMO_COMPANIES = [
  {
    ...
    collateral: {
      type: 'Industrial Land + Plant & Machinery',
      value: 280000000,
      coverage: 1.56
    },
    creditRating: 'BBB+',
  ...
  },
  {
    ...
    collateral: {
      type: 'Agricultural Land + Processing Plant',
      value: 120000000,
      coverage: 1.60
    },
    creditRating: 'A-',
  ...
  },
  {
    ...
    riskLevel: 'high',
    decision: 'Reject',
    requestedAmount: 500000000, // 50 Cr
    recommendedLimit: 0,
    score: 32,
    ...
    collateral: {
      type: 'Under-construction projects (disputed)',
      value: 150000000,
      coverage: 0.36
    },
    creditRating: 'D',
  ...
  },
  {
    ...
    riskLevel: 'low',
    decision: 'Approve',
    requestedAmount: 150000000, // 15 Cr
    ...
    collateral: {
      type: 'Commercial Office Space + FD Lien',
      value: 200000000,
      coverage: 1.43
    },
    creditRating: 'A',
  ...
  },
  {
    ...
    riskLevel: 'critical',
    decision: 'Reject',
    requestedAmount: 120000000, // 12 Cr
    recommendedLimit: 0,
    score: 18,
    ...
    collateral: {
      type: 'Old Machinery (Obsolete) + Personal Guarantee',
      value: 25000000,
      coverage: 0.21
    },
    creditRating: 'D',
...
function getDashboardStats() {
  const total = DEMO_COMPANIES.length;
  const approved = DEMO_COMPANIES.filter(c => c.decision === 'Approve').length;
  const rejected = total - approved;
  const avgScore = Math.round(DEMO_COMPANIES.reduce((s, c) => s + c.score, 0) / total);
  const fraudAlerts = DEMO_COMPANIES.reduce((s, c) => s + c.gstData.mismatchFlags.length, 0);
  return { total, approved, rejected, avgScore, fraudAlerts };
}
